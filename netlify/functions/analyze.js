...
function aiTimeoutMs() {
  // Previous versions capped this at 3.5s, which was too aggressive for real report requests.
  // Keep the default below common Netlify Function limits, but give OpenAI enough time to respond.
  const configured = Number(process.env.AI_TIMEOUT_MS || 25000);
  return Number.isFinite(configured) ? Math.max(5000, Math.min(configured, 45000)) : 25000;
}
  ...
  if (event.httpMethod === 'GET') {
    ...
    if (!key) {
      return {
        ...
        body: JSON.stringify({ ok: false, keyConfigured: false, model, timeoutMs: aiTimeoutMs(), version: 'v15', message: 'OPENAI_API_KEY is not configured for this Netlify Function.' })
      };
    }
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), version: 'v15', message: 'OpenAI key and model access look good.' })
      };
    } catch (error) {
      console.error(error);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: false, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), version: 'v15', message: safeError(error) })
      };
    }
  }
