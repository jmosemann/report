...
function aiTimeoutMs() {
  // Keep the default below common Netlify Function limits, but give OpenAI enough time to respond.
  const configured = Number(process.env.AI_TIMEOUT_MS || 55000);
  return Number.isFinite(configured) ? Math.max(5000, Math.min(configured, 55000)) : 55000;
}

function aiMaxOutputTokens() {
  // Earlier versions used 900 tokens, which could cut off the JSON mid-string.
  // This report schema has many required fields, so it needs a larger output budget.
  const configured = Number(process.env.AI_MAX_OUTPUT_TOKENS || 5000);
  return Number.isFinite(configured) ? Math.max(3200, Math.min(configured, 8000)) : 5000;
}
...
async function createStructuredReport(apiKey, model, metrics, transcriptText, timeoutMs) {
  ...
  const body = {
    ...
    instructions: [
      ...
      'Pages 11 and 12 receive the most AI-generated insight, but keep every sentence compact enough for a fixed PDF layout.',
      'Do not repeat the same takeaway title or executive summary across sections.'
    ].join('\n'),
    input: JSON.stringify({ metrics: compactMetricsForAi(metrics), transcriptText: String(transcriptText || '').slice(0, 900) }),
...
const schema = {
  ...
  properties: {
    executiveRead: { type: 'array', items: { type: 'string', maxLength: 105 }, minItems: 4, maxItems: 4 },
    ...
    leadershipPriorities: {
      ...
      items: {
        ...
        additionalProperties: false,
        properties: {
          area: { type: 'string', maxLength: 36 },
          priority: { type: 'string', maxLength: 120 },
          currentRead: { type: 'string', maxLength: 52 },
          status: { type: 'string', enum: ['WATCH', 'ACTION', 'GOOD'] },
          leadershipMove: { type: 'string', maxLength: 70 }
        },
        required: ['area', 'priority', 'currentRead', 'status', 'leadershipMove']
      }
    },
    executiveMandate: { $ref: '#/$defs/takeawayArray' },
    executiveTalkTrack: { type: 'array', items: { type: 'string', maxLength: 260 }, minItems: 4, maxItems: 4 },
    talkTrackSections: {
      type: 'array',
      minItems: 4,
      maxItems: 4,
      items: {
        ...
        properties: {
          ...
          hits: { type: 'array', items: { type: 'string', maxLength: 92 }, minItems: 2, maxItems: 2 },
          risks: { type: 'array', items: { type: 'string', maxLength: 92 }, minItems: 2, maxItems: 2 },
          questions: { type: 'array', items: { type: 'string', maxLength: 92 }, minItems: 2, maxItems: 2 }
        },
  ...
  required: ['executiveRead', 'executiveTakeaways', 'wigTakeaways', 'financialTakeaways', 'trendTakeaways', 'departmentTakeaways', 'inventoryTakeaways', 'fixedOpsTakeaways', 'oemTakeaways', 'leadershipPriorities', 'executiveMandate', 'executiveTalkTrack', 'talkTrackSections', 'finalNarrative'],
  $defs: {
    takeawayArray: {
      type: 'array',
      minItems: 3,
      ...
      items: {
        ...
        properties: {
          title: { type: 'string', maxLength: 30 },
          body: { type: 'string', maxLength: 105 }
        },
        required: ['title', 'body']
      }
    }
  ...
  if (event.httpMethod === 'GET') {
    ...
    if (!key) {
      return {
        ...
        body: JSON.stringify({ ok: false, keyConfigured: false, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v20', message: 'OPENAI_API_KEY is not configured for this Netlify Function.' })
      };
    }
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v20', message: 'OpenAI key and model access look good.' })
      };
    } catch (error) {
      console.error(error);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: false, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v20', message: safeError(error) })
      };
    }
  }
