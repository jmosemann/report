...
function aiTimeoutMs() {
  const configured = Number(process.env.AI_TIMEOUT_MS || 8000);
  return Number.isFinite(configured) ? Math.max(3000, Math.min(configured, 9000)) : 8000;
}
...
async function createStructuredReport(client, model, metrics, transcriptText, timeoutMs) {
  ...
  try {
    ...
    input: JSON.stringify({ metrics: compactMetricsForAi(metrics), transcriptText: String(transcriptText || '').slice(0, 12000) }),
...
export async function handler(event) {
  ...
  const client = new OpenAI({ apiKey: key, timeout: timeoutMs, maxRetries: 0 });
  ...
  for (const model of modelsToTry) {
    ...
    try {
      const response = await createStructuredReport(client, model, metrics, payload.transcriptText || '', remaining);
