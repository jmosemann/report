...
function aiTimeoutMs() {
  ...
  const configured = Number(process.env.AI_TIMEOUT_MS || 8500);
  return Number.isFinite(configured) ? Math.max(3000, Math.min(configured, 12000)) : 8500;
}
  ...
  if (event.httpMethod === 'GET') {
    ...
    if (!key) {
      return {
        ...
        body: JSON.stringify({ ok: false, keyConfigured: false, model, version: 'v14', message: 'OPENAI_API_KEY is not configured for this Netlify Function.' })
      };
    }
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, keyConfigured: true, model, version: 'v14', message: 'OpenAI key and model access look good.' })
      };
    } catch (error) {
      console.error(error);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: false, keyConfigured: true, model, version: 'v14', message: safeError(error) })
      };
    }
  }
