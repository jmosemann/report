...
function aiMaxOutputTokens() {
  ...
  return Number.isFinite(configured) ? Math.max(1800, Math.min(configured, 5000)) : 3200;
}
  ...
  if (event.httpMethod === 'GET') {
    ...
    if (!key) {
      return {
        ...
        body: JSON.stringify({ ok: false, keyConfigured: false, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v17', message: 'OPENAI_API_KEY is not configured for this Netlify Function.' })
      };
    }
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v17', message: 'OpenAI key and model access look good.' })
      };
    } catch (error) {
      console.error(error);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: false, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v17', message: safeError(error) })
      };
    }
  }
