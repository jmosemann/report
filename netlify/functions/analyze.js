...
function aiMaxOutputTokens() {
  ...
  const configured = Number(process.env.AI_MAX_OUTPUT_TOKENS || 3200);
  return Number.isFinite(configured) ? Math.max(2200, Math.min(configured, 4500)) : 3200;
}
...
async function createStructuredReport(apiKey, model, metrics, transcriptText, timeoutMs) {
  ...
  const body = {
    ...
    instructions: [
      ...
      'Pages 11 and 12 receive the most AI-generated insight, but every field must be short enough to fit a fixed PDF layout.',
      'Prefer concrete business language over long narrative. Use short sentence fragments when appropriate.',
...
const schema = {
  ...
  properties: {
    executiveRead: { type: 'array', items: { type: 'string', maxLength: 90 }, minItems: 4, maxItems: 4 },
    ...
    leadershipPriorities: {
      ...
      items: {
        ...
        properties: {
          ...
          priority: { type: 'string', maxLength: 88 },
          currentRead: { type: 'string', maxLength: 44 },
          status: { type: 'string', enum: ['WATCH', 'ACTION', 'GOOD'] },
          leadershipMove: { type: 'string', maxLength: 56 }
        },
    ...
    talkTrackSections: {
      ...
      items: {
        ...
        properties: {
          ...
          hits: { type: 'array', items: { type: 'string', maxLength: 68 }, minItems: 2, maxItems: 2 },
          risks: { type: 'array', items: { type: 'string', maxLength: 68 }, minItems: 2, maxItems: 2 },
          questions: { type: 'array', items: { type: 'string', maxLength: 68 }, minItems: 2, maxItems: 2 }
        },
  ...
  required: ['executiveRead', 'executiveTakeaways', 'wigTakeaways', 'financialTakeaways', 'trendTakeaways', 'departmentTakeaways', 'inventoryTakeaways', 'fixedOpsTakeaways', 'oemTakeaways', 'leadershipPriorities', 'executiveMandate', 'talkTrackSections', 'finalNarrative'],
  $defs: {
    takeawayArray: {
      type: 'array',
      minItems: 3,
