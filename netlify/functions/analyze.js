...
function aiTimeoutMs() {
  const configured = Number(process.env.AI_TIMEOUT_MS || 4500);
  return Number.isFinite(configured) ? Math.max(2500, Math.min(configured, 5500)) : 4500;
}
...
async function createStructuredReport(apiKey, model, metrics, transcriptText, timeoutMs) {
  const body = {
    ...
    input: JSON.stringify({ metrics: compactMetricsForAi(metrics), transcriptText: String(transcriptText || '').slice(0, 6000) }),
...
export async function handler(event) {
  ...
  const modelsToTry = Array.from(new Set([primaryModel, 'gpt-5.4-mini', 'gpt-5.4'].filter(Boolean)));
  const errors = [];
