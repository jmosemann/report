...
function aiTimeoutMs() {
  ...
  return Number.isFinite(configured) ? Math.max(5000, Math.min(configured, 50000)) : 30000;
}
...
async function createStructuredReport(apiKey, model, metrics, transcriptText, timeoutMs) {
  ...
  const body = {
    ...
    instructions: [
      ...
      'Pages 11 and 12 receive the most AI-generated insight: leadership priorities, executive mandate, department talk-track sections, and final narrative should be specific and useful.',
...
const schema = {
  ...
  properties: {
    executiveRead: { type: 'array', items: { type: 'string', maxLength: 130 }, minItems: 4, maxItems: 4 },
    ...
    leadershipPriorities: {
      ...
      items: {
        ...
        properties: {
          ...
          priority: { type: 'string', maxLength: 145 },
    ...
    executiveTalkTrack: { type: 'array', items: { type: 'string', maxLength: 360 }, minItems: 5, maxItems: 5 },
    talkTrackSections: {
      type: 'array',
      minItems: 4,
      maxItems: 4,
      items: {
        ...
        properties: {
          ...
          hits: { type: 'array', items: { type: 'string', maxLength: 112 }, minItems: 2, maxItems: 2 },
          risks: { type: 'array', items: { type: 'string', maxLength: 112 }, minItems: 2, maxItems: 2 },
          questions: { type: 'array', items: { type: 'string', maxLength: 112 }, minItems: 2, maxItems: 2 }
        },
  ...
  if (event.httpMethod === 'GET') {
    ...
    if (!key) {
      return {
        ...
        body: JSON.stringify({ ok: false, keyConfigured: false, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v18', message: 'OPENAI_API_KEY is not configured for this Netlify Function.' })
      };
    }
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v18', message: 'OpenAI key and model access look good.' })
      };
    } catch (error) {
      console.error(error);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: false, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), maxOutputTokens: aiMaxOutputTokens(), version: 'v18', message: safeError(error) })
      };
    }
  }
