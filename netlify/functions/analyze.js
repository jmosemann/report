  ...
  if (event.httpMethod === 'GET') {
    ...
    if (!key) {
      return {
        ...
        body: JSON.stringify({ ok: false, keyConfigured: false, model, version: 'v12', message: 'OPENAI_API_KEY is not configured for this Netlify Function.' })
      };
    }
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, keyConfigured: true, model, version: 'v12', message: 'OpenAI key and model access look good.' })
      };
    } catch (error) {
      console.error(error);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: false, keyConfigured: true, model, version: 'v12', message: safeError(error) })
      };
    }
  }
