...
function aiMaxOutputTokens() {
  // v25 uses a compact AI schema, so a smaller output budget is enough and avoids runaway completions.
...
function inflateCompactAi(compact = {}, metrics = {}) {
  ...
  const talkTrackSections = fallback.talkTrackSections.map((section, i) => ({
    ...section,
    hits: [truncateText(talkGuidance[i] || section.hits[0], 74), section.hits[1]]
  }));
...
async function createStructuredReport(apiKey, model, metrics, transcriptText, timeoutMs) {
  ...
  const body = {
    ...
    instructions: [
      ...
      'Each string should be one short sentence or sentence fragment.'
    ].join('\n'),
    input: JSON.stringify({
      metrics: compactMetricsForAi(metrics),
      transcriptContext: String(transcriptText || '').replace(/\s+/g, ' ').slice(0, 350)
    }),
  ...
  if (data?.status === 'incomplete') {
    ...
    const err = new Error(`OpenAI response was incomplete (${reason}). v25 compact schema should normally fit; reduce AI_MAX_OUTPUT_TOKENS only if timeouts occur or use gpt-4.1-mini.`);
...
async function innerHandler(event) {
  ...
  for (const model of modelsToTry) {
    ...
    try {
      ...
      const inflated = inflateCompactAi(parsed, metrics);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...inflated, _model: model })
      };
    } catch (error) {
      console.error(error);
