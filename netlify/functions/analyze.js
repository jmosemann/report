...
function aiTimeoutMs() {
  ...
  return Number.isFinite(configured) ? Math.max(5000, Math.min(configured, 45000)) : 30000;
}
  ...
  if (event.httpMethod === 'GET') {
    ...
    if (!key) {
      return {
        ...
        body: JSON.stringify({ ok: false, keyConfigured: false, model, timeoutMs: aiTimeoutMs(), version: 'v16', message: 'OPENAI_API_KEY is not configured for this Netlify Function.' })
      };
    }
    try {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), version: 'v16', message: 'OpenAI key and model access look good.' })
      };
    } catch (error) {
      console.error(error);
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: false, keyConfigured: true, model, timeoutMs: aiTimeoutMs(), version: 'v16', message: safeError(error) })
      };
    }
  }
