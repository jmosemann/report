...
function selectTranscriptContext(transcriptText = '', maxChars = 12000) {
  const text = String(transcriptText || '').replace(/\s+/g, ' ').trim();
  if (!text) return '';
  const keywords = /\b(wig|wildly important|goal|priority|owner|said|flagged|concern|issue|challenge|opportunity|market share|share loss|registration|brp|can[- ]?am|sea[- ]?doo|unit count|volume|lead|credit|approval|turndown|down payment|lender|f&i|finance|inventory|floorplan|aged|cash|capital|service|bay|technician|capacity|facility|building|parts|accessor|budget|seasonality|ncm|crm|lightspeed|system|scorecard|manager|management|leadership|training|trust|legacy|relationship)\b/i;
  const chunks = text
    .split(/(?<=[.!?])\s+|\n+/)
  ...
  const scored = chunks.map((chunk, idx) => {
    ...
    let score = 0;
    if (keywords.test(chunk)) score += 5;
    if (/market share|share loss|registration|brp|can[- ]?am|sea[- ]?doo/i.test(chunk)) score += 8;
    if (/credit|approval|turndown|down payment|lender|f&i|finance/i.test(chunk)) score += 7;
    if (/unit count|volume|lead|traffic|swings/i.test(chunk)) score += 6;
    if (/budget|seasonality|ncm|monthly target/i.test(chunk)) score += 6;
    if (/manager|management|leadership|scorecard|ownership|training/i.test(chunk)) score += 6;
    if (/service|bay|technician|capacity|facility|building|parts/i.test(chunk)) score += 5;
    if (/inventory|floorplan|aged|cash|capital|turn/i.test(chunk)) score += 5;
    if (chunk.length > 240) score -= 1;
  ...
  const selected = scored
    ...
    .map((item) => item.chunk);
  const context = selected.length ? selected.join(' ') : text.slice(0, maxChars);
  return context.slice(0, maxChars);
}

function compactForAi(metrics = {}) {
  const compactService = metrics.service ? {
    roCount: metrics.service.roCount,
    warrantyRos: metrics.service.warrantyRos,
    warrantyRoRate: metrics.service.warrantyRoRate,
    revenue: metrics.service.revenue,
    cost: metrics.service.cost,
    grossProfit: metrics.service.grossProfit,
    byMonth: metrics.service.byMonth
  } : {};

  const compactParts = metrics.parts ? {
    invoiceCount: metrics.parts.invoiceCount,
    revenue: metrics.parts.revenue,
    cost: metrics.parts.cost,
    grossProfit: metrics.parts.grossProfit
  } : {};
  ...
  return {
    ...
    monthly: Array.isArray(metrics.monthly) ? metrics.monthly.map((m) => ({
      month: m.month,
      revenue: m.revenue,
    ...
    departments: Array.isArray(metrics.departments) ? metrics.departments.map((d) => ({
    ...
    oems: Array.isArray(metrics.oems) ? metrics.oems.map((o) => ({
      make: o.make,
      units: o.units,
      revenue: o.revenue,
...
async function requestAiAnalysis(metrics, transcriptText) {
  const body = {
    metrics: compactForAi(metrics),
    transcriptText: selectTranscriptContext(transcriptText, 12000)
  };
...
async function exportReportPdf() {
  ...
  // Wait for web fonts and the HerohubOS logo to finish loading before opening print.
  if (document.fonts?.ready) {
    await document.fonts.ready;
  }
  const images = Array.from(document.querySelectorAll('.report-page img'));
  ...
  window.print();
  setTimeout(() => document.body.classList.remove('exporting-pdf'), 1000);
}
...
export default function App() {
  ...
  async function handleExport() {
    setError('');
    setStatus('Opening vector PDF print dialog…');
    try {
      await exportReportPdf();
      setStatus('Print dialog opened. Choose “Save as PDF” to create a vector PDF with selectable text.');
    } catch (err) {
      setError(err.message || String(err));
      setStatus('');
    }
  ...
  return (
    <div className="app-shell">
      <header className="app-hero">
        <div>
          <img className="app-logo-img" src={`${import.meta.env.BASE_URL || '/'}herohub-os-logo.svg`} alt="HerohubOS" />
          <h1>Executive Report Builder</h1>
          <p>Upload dealer data and onboarding transcripts, select the reporting period, run AI-assisted analysis, and export a HerohubOS-style vector landscape PDF.</p>
        </div>
      ...
      <main className="builder-grid">
        ...
        <section className="builder-card">
          ...
          {reportData ? <button className="secondary" onClick={handleExport}>Export Vector PDF</button> : null}
          ...
          <>
            <div className="preview-toolbar">
              <div>
                ...
                <p>Pages below print one-for-one into a vector landscape PDF with selectable text.</p>
              </div>
              <button className="primary" onClick={handleExport}>Export Vector PDF</button>
            </div>
