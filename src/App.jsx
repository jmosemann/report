...
export default function App() {
  ...
  const [reportStart, setReportStart] = useState(addMonths(currentMonthKey(), -11));
  const [reportEnd, setReportEnd] = useState(currentMonthKey());
  const [dataStart, setDataStart] = useState(addMonths(currentMonthKey(), -11));
  const [dataEnd, setDataEnd] = useState(currentMonthKey());
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setStatus(ai?._warning ? `Report created. AI fallback used: ${ai._warning}` : 'Report created. Review the preview, then export the PDF.');
  ...
  return (
    <div className="app-shell">
      <header className="app-hero">
        <div>
          <div className="app-logo">HER<span>OHUB OS</span></div>
