...
function pickTakeawayIcon(item = {}, fallback = Sparkles) {
  ...
  if (/service|fixed ops|repair|ro\b|bay|technician|throughput|capacity/.test(text)) return Wrench;
  if (/inventory|floorplan|payable|capital|aged|stock|cash planning/.test(text)) return PackageSearch;
  if (/f&i|finance|pvr|penetration|acceptance|script|role/.test(text)) return ShieldCheck;
  if (/crm|system|lightspeed|data|scorecard|reconciliation|reporting/.test(text)) return Database;
  if (/trend|monthly|season|volume|growth|trajectory|momentum/.test(text)) return TrendingUp;
  if (/margin|profit|nop|gross|cash|revenue|expense|cost|money/.test(text)) return CircleDollarSign;
  if (/goal|wig|priority|rigor|discipline|inspection|focus|move/.test(text)) return Crosshair;
...
function EmptyChart({ label = 'No chart data available' }) {
  return (
    <svg className="chart-svg" viewBox="0 0 720 250" role="img" aria-label={label} preserveAspectRatio="none">
...
function TrendChart({ data, lines = ['revenue', 'nop'] }) {
  ...
  return (
    <svg className="chart-svg" viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Trend chart" preserveAspectRatio="none">
      ...
      {rows.map((row, idx) => {
        ...
        return show ? <text key={`${row.month}-${idx}`} x={tx} y={H - 27} textAnchor="middle" className="chart-tick">{row.month}</text> : null;
      })}
...
function BarMoneyChart({ data, dataKey, name, color = COLORS.teal, yLabel = 'Dollars ($)', valueType = 'money' }) {
  ...
  return (
    <svg className="chart-svg" viewBox={`0 0 ${W} ${H}`} role="img" aria-label={name || 'Bar chart'} preserveAspectRatio="none">
      <rect x="0" y="0" width={W} height={H} fill="#fff" />
      {ticks.map((tick) => {
        const ty = y(tick);
        return (
      ...
      {rows.map((row, idx) => {
        ...
        return (
          <g key={`${label}-${idx}`}>
            ...
            <text x={cx} y={H - 28} textAnchor="middle" className="chart-tick chart-x-tick">{label}</text>
          </g>
        );
      })}
  ...
  return (
    <Page className="cover-page">
      <div className="cover-content">
        <img className="cover-logo" src="/herohub-os-logo.svg" alt="HerohubOS" />
