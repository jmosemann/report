...
function TrendChart({ data, lines = ['revenue', 'nop'] }) {
  ...
  const W = 720;
  const H = 280;
  const m = { top: 42, right: 24, bottom: 70, left: 78 };
  ...
  return (
    <svg className="chart-svg" viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Trend chart" preserveAspectRatio="xMidYMid meet">
      ...
      {rows.map((row, idx) => {
        ...
        return show ? <AxisLabel key={`${row.month}-${idx}`} x={tx} y={H - 46} label={row.month} maxChars={8} /> : null;
      })}
      <text x={m.left + plotW / 2} y={H - 18} textAnchor="middle" className="chart-axis-label">Month</text>
...
function BarMoneyChart({ data, dataKey, name, color = COLORS.teal, yLabel = 'Dollars ($)', valueType = 'money' }) {
  ...
  const W = 720;
  const H = 280;
  const m = { top: 30, right: 22, bottom: 72, left: 78 };
  const plotW = W - m.left - m.right;
  const plotH = H - m.top - m.bottom;
      ...
      {rows.map((row, idx) => {
        ...
        return (
          <g key={`${label}-${idx}`}>
            ...
            <AxisLabel x={cx} y={H - 48} label={label} maxChars={rows.length > 6 ? 8 : 12} />
          </g>
        );
      })}
      <text x={m.left + plotW / 2} y={H - 18} textAnchor="middle" className="chart-axis-label">Category</text>
...
function SectionBullets({ label, items, kind }) {
  ...
  return (
    <div className={`talk-section ${kind}`}>
      <h5>{label}</h5>
      <ul>
        {items.slice(0, 2).map((item, idx) => <li key={`${label}-${idx}`}>{item}</li>)}
      </ul>
    </div>
  );
}
