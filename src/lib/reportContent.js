...
function compactText(value, max = 140) {
  ...
  if (!text) return '';
  if (text.length <= max) return text.replace(/\s*(?:\.\.\.|…)+\s*$/, '').replace(/[,:;\-–—]+$/, '').trim();

  const hardLimit = Math.max(max, Math.min(text.length, Math.round(max * 1.45)));
  ...
  if (sentenceMatches.length) {
    ...
    if (end >= Math.max(28, max * 0.45)) return soft.slice(0, end).trim();
  }
  const clauseCut = Math.max(soft.lastIndexOf('; '), soft.lastIndexOf(', '), soft.lastIndexOf(' — '), soft.lastIndexOf(' - '));
  if (clauseCut >= Math.max(34, max * 0.55)) return soft.slice(0, clauseCut).replace(/[,:;\-–—]+$/, '').trim();
  const wordCut = soft.lastIndexOf(' ');
  return (wordCut > 28 ? soft.slice(0, wordCut) : soft).replace(/[,:;\-–—]+$/, '').trim();
}

function compactSentence(value, max = 140) {
  return completeSentence(compactText(value, max));
}
...
function normalizeTakeawayList(input, fallback, used = new Set()) {
  ...
  const candidates = [...(source || []), ...(fallback || []),
    { title: 'Focus the next review', body: 'Turn this page into one owner, one action, and one inspection.' },
    { title: 'Keep it measurable', body: 'Use the next meeting to confirm movement, variance, and accountability.' },
    { title: 'Connect the work', body: 'Review the metric, the behavior, and the follow-up rhythm together.' }
  ];
  for (const item of candidates) {
    if (!item || clean.length >= 3) break;
    const title = compactText(item.title, 34);
    const body = compactSentence(item.body, 112);
...
function normalizeExecutiveRead(input, fallback) {
  const source = Array.isArray(input) && input.length ? input : fallback;
  const clean = source.slice(0, 4).map((line, idx) => compactSentence(line || fallback[idx], 128));
  while (clean.length < 4 && fallback[clean.length]) clean.push(compactSentence(fallback[clean.length], 140));
...
function normalizePriorityList(input, fallback) {
  ...
  const clean = source.slice(0, 5).map((item, idx) => {
    ...
    return {
      ...
      priority: compactSentence(item?.priority || backup.priority, 86),
      currentRead: compactText(item?.currentRead || backup.currentRead, 44),
      status: ['WATCH', 'ACTION', 'GOOD'].includes(status) ? status : 'WATCH',
      leadershipMove: compactSentence(item?.leadershipMove || backup.leadershipMove, 74)
    };
  });
  while (clean.length < 5 && fallback[clean.length]) {
    ...
    clean.push({
      ...
      priority: compactSentence(item.priority, 86),
      currentRead: compactText(item.currentRead, 44),
      status: item.status,
      leadershipMove: compactSentence(item.leadershipMove, 74)
    });
  }
...
function normalizeBulletList(input, fallback, maxItems = 3, maxLen = 118) {
  ...
  for (const item of source || []) {
    ...
    const text = compactSentence(item, maxLen);
...
function normalizeTalkSections(input, fallback) {
  ...
  const out = source.slice(0, 4).map((section, idx) => {
    ...
    return {
      ...
      hits: normalizeBulletList(section?.hits, backup.hits, 2, 92),
      risks: normalizeBulletList(section?.risks, backup.risks, 2, 92),
      questions: normalizeBulletList(section?.questions, backup.questions, 2, 92)
    };
  });
