...
function normalizePriorityList(input, fallback) {
  ...
  const clean = source.slice(0, 5).map((item, idx) => {
    ...
    return {
      ...
      priority: compactText(item?.priority || backup.priority, 76),
      currentRead: compactText(item?.currentRead || backup.currentRead, 44),
      status: ['WATCH', 'ACTION', 'GOOD'].includes(status) ? status : 'WATCH',
      leadershipMove: compactText(item?.leadershipMove || backup.leadershipMove, 64)
    };
  });
  while (clean.length < 5 && fallback[clean.length]) {
    ...
    clean.push({
      ...
      priority: compactText(item.priority, 76),
      currentRead: compactText(item.currentRead, 44),
      status: item.status,
      leadershipMove: compactText(item.leadershipMove, 64)
    });
  }
...
function normalizeTalkSections(input, fallback) {
  ...
  const out = source.slice(0, 4).map((section, idx) => {
    ...
    return {
      ...
      hits: normalizeBulletList(section?.hits, backup.hits, 2, 58),
      risks: normalizeBulletList(section?.risks, backup.risks, 2, 58),
      questions: normalizeBulletList(section?.questions, backup.questions, 2, 58)
    };
  });
