...
function compactText(value, max = 140) {
  ...
  const slice = text.slice(0, Math.max(0, max));
  const sentenceCut = Math.max(slice.lastIndexOf('. '), slice.lastIndexOf('? '), slice.lastIndexOf('! '));
  const wordCut = Math.max(slice.lastIndexOf('; '), slice.lastIndexOf(', '), slice.lastIndexOf(' '));
  const cut = sentenceCut > max * 0.45 ? sentenceCut + 1 : wordCut;
...
function normalizeTakeawayList(input, fallback, used = new Set()) {
  ...
  for (const item of candidates) {
    ...
    const body = compactText(item.body, 100);
...
function normalizeExecutiveRead(input, fallback) {
  const source = Array.isArray(input) && input.length ? input : fallback;
  const clean = source.slice(0, 4).map((line, idx) => compactText(line || fallback[idx], 112));
...
function normalizePriorityList(input, fallback) {
  ...
  const clean = source.slice(0, 5).map((item, idx) => {
    ...
    return {
      ...
      priority: compactText(item?.priority || backup.priority, 118),
      currentRead: compactText(item?.currentRead || backup.currentRead, 48),
      status: ['WATCH', 'ACTION', 'GOOD'].includes(status) ? status : 'WATCH',
      leadershipMove: compactText(item?.leadershipMove || backup.leadershipMove, 72)
    };
  });
  while (clean.length < 5 && fallback[clean.length]) {
    ...
    clean.push({
      ...
      priority: compactText(item.priority, 118),
      currentRead: compactText(item.currentRead, 48),
      status: item.status,
      leadershipMove: compactText(item.leadershipMove, 72)
    });
  }
...
function normalizeTalkSections(input, fallback) {
  ...
  const out = source.slice(0, 4).map((section, idx) => {
    ...
    return {
      ...
      hits: normalizeBulletList(section?.hits, backup.hits, 2, 74),
      risks: normalizeBulletList(section?.risks, backup.risks, 2, 74),
      questions: normalizeBulletList(section?.questions, backup.questions, 2, 74)
    };
  });
