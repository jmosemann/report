...
## v24 AI schema reduction

V24 fixes repeated `OPENAI_INCOMPLETE` errors by reducing the AI schema. AI now focuses on the executive read and the final two pages: Leadership Priorities, Executive Mandate, Executive Talk Track, and Final Executive Narrative. Earlier page right-rail copy is generated deterministically from the metrics and then de-duplicated client-side.
...
After deploy, verify `/.netlify/functions/analyze` returns `version: "v24"`.

## v25 compact AI insight schema

v25 changes the Netlify AI function so it no longer asks OpenAI to generate the entire final page structure directly. Instead, OpenAI generates a compact set of short insight strings, and the function expands those into the report structure. This keeps the last two pages AI-informed while avoiding `OPENAI_INCOMPLETE` max-output errors.
...
After deploy, verify `/.netlify/functions/analyze` returns `version: "v25"`.


## v26 transcript intelligence layer

v26 adds a transcript intelligence layer before report generation. The browser now sends a scored transcript context rather than only the first few lines of the transcript. The Netlify Function extracts owner-stated themes such as market share, qualified unit volume, credit/F&I, budget and seasonality, leadership scorecards, systems/CRM, service capacity, inventory/capital, parts attachment, trust/legacy, and deal-structure discipline.

Those extracted themes now directly inform:

- the Wildly Important Goals table
- WIG right-rail takeaways
- Leadership Priorities
- Executive Mandate
- Executive Talk Track sections
...
After deploy, verify `/.netlify/functions/analyze` returns `version: "v26"`.


## v30 deploy note

The vector export uses the browser print engine and defines a 1600x900 CSS page with a matching custom 16:9 `@page` size. When saving the report as PDF, use:

- Destination: Save as PDF
- Layout: Landscape, if the dialog offers it
- Margins: None
- Scale: 100
- Background graphics: On

If Chrome still shows a portrait preview, choose the print dialog's Landscape layout once; the CSS page size will handle the report dimensions from there.

## v30 dependency install note

This package intentionally does not include package-lock.json. Netlify will install dependencies from package.json using the public npm registry configured in .npmrc. If a package-lock.json appears after local testing, do not commit it unless it was generated from the public npm registry.

## v32 talk-track sentence completion
- Executive Talk Track bullets are now normalized to end as complete sentences.
- The AI prompt now explicitly forbids dangling endings like “with,” “for,” “to,” “and,” and “so.”
- Talk-card text sizing was tightened slightly so complete sentences can fit without visible mid-sentence cutoffs.
- Function test should return `version: "v32"`.
